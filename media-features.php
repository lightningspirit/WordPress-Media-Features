...
class WP_Media_Features {
	...
	public static function init() {
		...
		/** Restrict and manage WP_Media_List_Table presentation */
		add_action( 'load-upload.php', array( 'WP_Media_Features', 'admin_head_upload' ) );
		...
		add_filter( 'media_row_actions', array( 'WP_Media_Features', 'media_row_actions' ), 10, 3 );
		add_action( 'wp_ajax_get_media_attach_categories', array( 'WP_Media_Features', 'get_media_attach_categories' ) );
		//add_filter( 'bulk_actions-upload', array( 'WP_Media_Features', 'bulk_actions_upload' ) );
		//add_filter( 'wp_redirect', array( 'WP_Media_Features', 'bulk_actions_do' ) );
		
		/** Add and customize taxonomy */
		...
		/** Add controllers to options-media.php */
	...
	public static function do_upgrade() {
		update_option( 'media_features_plugin_version', '0.2' );
	 ...
	 * @return array
	 */
	public static function admin_head_upload() {
		wp_enqueue_script( 'jquery-ui-tabs' );
		wp_enqueue_script( 'media-features', plugin_dir_url( __FILE__ ) . '/js/media-features.js', array( 'jquery' ), '1.0' );
		wp_localize_script( 'media-features', 'media_features', array(
			'detach' 		=> __( 'Detach', 'media-features' ),
			'categorize' 	=> __( 'Categorize', 'media-features' ),
			)
		);
		
		// Save terms to posts
		if ( isset( $_REQUEST['media_category_save_categories'] ) ) {
			$post_ids = explode( ',', $_REQUEST['media_category_save_categories_posts_ids'] );
			$term_ids = $_REQUEST['media_category_save_categories_terms_ids'];
			
			foreach ( $post_ids as $post_id ) {
				wp_set_post_terms( (int) $post_id, $term_ids, 'media_category' );
				
			}
			
			
			wp_redirect( add_query_arg( 'attach_categories', '1', remove_query_arg( array( 'media_category_save_categories', 'media_category_save_categories_posts_ids', 'media_category_save_categories_terms_ids' ), stripslashes( $_SERVER['REQUEST_URI'] ) ) ) );
			
		}
		
		if ( isset( $_REQUEST['attach_categories'] ) ) {
			add_action( 'admin_notices', array( 'WP_Media_Features', 'notice_attach_categories' ) );
	 ...
	 * @return void
	 */
	public static function notice_attach_categories() {
		echo '<div class="updated"><p>';
		printf( __( 'Categories attached to media files.', 'media-features' ), 'options-media.php' );
		echo '</p></div>';
	...
	public static function media_upload_filter_dropdowns() {
		...
		?>
		<div class="alignright actions" style="margin:-1px 0 auto 10px ;">
			<input id="attach-categories-do" type="button" class="button" value="Attach Categories" />
		</div>


		<div id="attach-categories" class="find-box" style="display:none;">
			<div id="attach-categories-head" class="find-box-head">
				<?php _e( 'Attach Categories to Media Files', 'media-features' ); ?>
			</div>
			
			<div id="attach-categories-inside" class="find-box-inside" style="background:transparent;width:95%;padding:0 5px;position:relative;">
				
			</div>
			
			<input id="media_category_save_categories" type="hidden" name="media_category_save_categories" value="true">
			<input id="media_category_save_categories_posts_ids" type="hidden" name="media_category_save_categories_posts_ids" value="">
			<input id="media_category_save_categories_terms_ids" type="hidden" name="media_category_save_categories_terms_ids" value="">
		
			<div class="find-box-buttons">
				<input id="attach-categories-close" type="button" class="button alignleft" value="<?php esc_attr_e( 'Close' ); ?>" />
				<?php submit_button( __( 'Save' ), 'button-primary alignright', 'attach-categories-submit', false ); ?>	
			</div>
	...
	/** 
	 * Add row actions for media handle
	 * 
	 * @since 0.2
	 * 
	 * @param array $actions
	 * @return array
	 */
	public static function media_row_actions( $actions, $post, $detached ) {		
		if ( $detached )
			return $actions;
		
		return array(
			'edit' 		=> $actions['edit'],
			'category' 	=> sprintf( '<a href="#the-list" onclick="attachCategories.open( \'media[]\', \'%d\' );return false;" class="hide-if-no-js">%s</a>', $post->ID, __( 'Attach categories', 'media-features' ) ),
	...
	/** 
	 * AJAX Get postbox media categories according to actual post
	 * 
	 * @since 0.2
	 * 
	 * @return void
	 */
	public static function get_media_attach_categories() {
		...
		ob_start();
		WP_Media_Features::media_categories_box( $post_id );
		$data = ob_get_clean();
		
		$ajax = new WP_Ajax_Response( compact( 'data' ) );
		$ajax->send();
	...
	function media_categories_box( $post_id = 0 ) {
		...
		<div id="taxonomy-media_category" class="categorydiv">
			<ul id="media_category-tabs" class="category-tabs">
				<li class="tabs">
					<a href="#media_category-all"><?php echo $tax->labels->all_items; ?></a>
				</li>
				<li class="hide-if-no-js">
					<a href="#media_category-pop"><?php _e( 'Most Used' ); ?></a>
				</li>
			</ul>
			...
			<div id="media_category-pop" class="tabs-panel" style="display: none;">
				<ul id="media_categorychecklist-pop" class="categorychecklist form-no-clear" >
					<?php $popular_ids = wp_popular_terms_checklist( 'media_category' ); ?>
				</ul>
			</div>
			...
			<div id="media_category-all" class="tabs-panel">
				<ul id="media_categorychecklist" data-wp-lists="list:media_category" class="categorychecklist form-no-clear">
					<?php wp_terms_checklist( $post_id, array( 'taxonomy' => 'media_category', 'popular_cats' => $popular_ids ) ) ?>
				</ul>
			</div>
		</div>
